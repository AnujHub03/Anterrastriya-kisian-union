...
const LeadershipDirectory = () => {
  ...
  const getTierStyles = (tier) => {
    switch(tier) {
      case 'State': return { text: 'text-accent', bg: 'bg-accent/10 border-accent/20', badge: 'bg-gradient-to-r from-accent to-accent/80', stripe: 'bg-gradient-to-b from-accent via-accent/80 to-accent' }
      case 'District': return { text: 'text-secondary', bg: 'bg-secondary/10 border-secondary/20', badge: 'bg-gradient-to-r from-secondary to-secondary/80', stripe: 'bg-gradient-to-b from-secondary via-secondary/80 to-secondary' }
      default: return { text: 'text-base-content', bg: 'bg-neutral/10 border-neutral/20', badge: 'bg-gradient-to-r from-neutral to-neutral/80', stripe: 'bg-gradient-to-b from-neutral via-neutral/80 to-neutral' }
    }
  }
    ...
    <div className="py-8 max-w-7xl mx-auto space-y-10 px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative rounded-[2.5rem] overflow-hidden border border-white/20 dark:border-white/10 shadow-2xl">
        ...
        <div className="relative z-10 p-10 sm:p-16 flex flex-col items-center text-center">
          ...
          <h2 className="text-4xlb text-slate-100 sm:text-5xl font-black tracking-tight leading-tight drop-shadow-sm">
            {t.leadership.title}
          </h2>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/60 dark:bg-base-900/60 backdrop-blur-xl p-3 rounded-2xl border border-white/40 dark:border-white/10 shadow-lg sticky top-4 z-20">
        ...
        <div className="flex flex-wrap gap-2 p-1 bg-base-100/50 dark:bg-base-900/50 rounded-xl">
          {['All', 'State', 'District', 'City'].map((tier) => (
            <button 
              key={tier} 
              onClick={() => setActiveTier(tier)} 
              className={`btn btn-sm rounded-lg px-5 font-bold transition-all duration-300 border-none ${
                activeTier === tier 
                  ? (tier === 'All' ? 'bg-primary text-primary-content shadow-lg shadow-primary/25' : tier === 'State' ? 'bg-accent text-accent-content shadow-lg shadow-accent/25' : tier === 'District' ? 'bg-secondary text-secondary-content shadow-lg shadow-secondary/25' : 'bg-neutral text-neutral-content shadow-lg shadow-neutral/25')
                  : 'bg-transparent hover:bg-base-200/50 text-base-content/70'
              }`}
            >
              {t.leadership[`filter${tier}`]}
            </button>
          ))}
        </div>
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-6 pl-4">
                    <div>
                      <h3 className="text-2xl font-black text-base-content tracking-tight group-hover:text-primary transition-colors duration-300">{leader.name}</h3>
                      <p className="text-sm font-semibold text-base-content/60 mt-1 flex items-center gap-2">
